    ...
    try {
      const listingPayload = {
        ...
        nearby: form.nearbyText
          ...
          .map((line) => line.trim())
          .filter(Boolean),
        available_from: form.availableFrom.trim() || null,
      };
                        ...
                        {previewImages.length ? (
                          previewImages.map((image, index) => (
                            <div key={`${image}-${index}`} className="group relative overflow-hidden border-2 border-[#5C3A21]/20 bg-white">
                              ...
                              {index === 0 && (
                                <span className="absolute left-2 top-2 border border-[#2C1810] bg-[#FAF3E0] px-2 py-1 text-[11px] font-bold uppercase tracking-[0.12em]">
                                  Main
                                </span>
                              )}
                              <button
                ...
                {(mainImage ? [mainImage, ...previewImages.slice(1, 4)] : Array.from({ length: 4 }).map(() => "")).map((image, index) => (
                  <div key={`${image || "empty"}-${index}`} className="overflow-hidden border border-[#F4E8C1]/20 bg-[#FAF3E0]/10">
                    {image ? (
                      ...
                      <div className="flex h-24 items-center justify-center text-xs uppercase tracking-[0.2em] text-[#F4E8C1]/70">
                        Empty
                      </div>
                    )}
                  </div>
